...
set.seed(42)
library(rusquant)
getSymbols('SPY', from='2017-12-01')
chartSeries(SPY$SPY.Close, theme = "wsj", TA = 'addBBands()')
...
# see plot of localMaxMin$maxmin against SPY
kWW <- 15
...
localMax <- rollapply(cl,
                      ...
                      by.column = F,
                      FUN = function(x)
                          return(x[which.max(x[, 'SPY.Close']), c('SPY.Close', 'dt')]))
...
localMin <- rollapply(cl,
                      ...
                      by.column = F,
                      FUN = function(x)
                          return(x[which.min(x[, 'SPY.Close']), c('SPY.Close', 'dt')]))
...
plot(as.zoo(SPY$SPY.Close))
par(new = T)
lines(as.zoo(localMaxMin$maxmin), col = "green")
...
dummySPY <- xts(rep(0, length = nrow(SPY)), order.by = index(SPY))
localMaxMin <- cbind(localMaxMin, dummySPY)
localMaxMin$SPY <- Cl(SPY)
localMaxMin$retSPY <- ret
# # start rev rest
...
(periodReturn5 <- prod(1+localMaxMin$retSPY*localMaxMin$longShort, na.rm = T) - 1)
chartSeries(SPY, theme = "wsj")
